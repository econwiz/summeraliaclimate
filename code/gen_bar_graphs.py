...
def main():
    ...
    mort_stats = xr.concat(mort_list, dim="gwl")
    gdp_stats  = xr.concat(gdp_list,  dim="gwl")

    f_int_mort = mort_stats["f_internal"].mean(dim=("lat","lon")).values
    f_mod_mort = mort_stats["f_model"].mean(dim=("lat","lon")).values
    f_int_gdp  = gdp_stats["f_internal"].mean(dim=("lat","lon")).values
    f_mod_gdp  = gdp_stats["f_model"].mean(dim=("lat","lon")).values
