...
use "$CODE/regressions/prep_panels/global_mortality_panel_public_ERA5_025.dta", clear

drop if inlist(iso,"USA","CHN")
...
replace sample = 0 if mi(tavg_poly_1_ERA5_025)
*replace sample = 0 if mi(prcp_poly_1_ERA5_025)
replace sample = 0 if mi(loggdppc_adm1_avg)
replace sample = 0 if mi(lr_tavg_ERA5_025_adm1_avg)
...
save "$OUTPUT/panel_prepped_for_regressions_ERA5.dta", replace
display "✅ Saved ERA5 panel to: $OUTPUT/panel_prepped_for_regressions_ERA5.dta"
