#!/usr/bin/env Rscript
# ------------------------------------------------------------------------------
# This merges new GMFD observational temperature and precipitation data 
# with the original Carleton et al. (2022) mortality panel.
#
# Input:
# - Original mortality panel (.dta)
# - Aggregated GMFD-by-region CSV with temperature and precipitation
#
# Output:
# - New panel with GMFD vars written to `prep_panels/`
#
# ------------------------------------------------------------------------------
...
obs_csv   <- "/user/ab5405/summeraliaclimate/code/regressions/0_generate_obs_data/obs_csvs/GMFD_by_region_year.csv"
panel_out <- "/user/ab5405/summeraliaclimate/code/regressions/prep_panels/global_mortality_panel_public_GMFD.dta"
...
panel <- read_dta(panel_in) %>%
  ...
  select(-matches("gmfd|best", ignore.case = TRUE))
...
diag <- merged %>% summarise(
  total_rows    = n(),
  with_climate  = sum(!is.na(tavg_poly_1_GMFD)),
  pct_populated = mean(!is.na(tavg_poly_1_GMFD)) * 100,
