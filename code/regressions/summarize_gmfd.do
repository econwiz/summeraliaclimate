...
foreach i of numlist 1/4 {
    estimates use "`sterdir'/pooled_response_spec`i'_public_GMFD.ster"
...
esttab spec1 spec2 spec3 spec4, ///
    ...
    title("GMFD: Spec 1–4 (with intercept)") ///
    nonotes compress

* 5. Export to CSV (including _cons)
esttab spec1 spec2 spec3 spec4 using "$OUTPUT/age_combined_GMFD/era5_025_results_table.csv", ///
    se label ///
    star(* 0.1 ** 0.05 *** 0.01) ///
    keep(_cons tavg_poly_*) ///
    order(_cons tavg_poly_*) ///
...
esttab spec1 spec2 spec3 spec4 using "$OUTPUT/age_combined_GMFD/era5_025_results_table.tex", ///
    se label ///
    star(* 0.1 ** 0.05 *** 0.01) ///
    title("GMFD Mortality Regression Results (Specs 1–4)") ///
    keep(_cons tavg_poly_*) ///
    order(_cons tavg_poly_*) ///
    replace

di "✅ GMFD tables (with constants) saved in $OUTPUT/age_combined_GMFD/"
