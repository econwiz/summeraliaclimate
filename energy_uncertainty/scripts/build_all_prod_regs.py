...
# -*- coding: utf-8 -*-
"""
build_PRODUCT_country_climate_multi.py - FIXED VERSION

CRITICAL FIX: Precipitation now uses MONTHLY data and takes MEAN (not sum)
Following Carleton et al. 2022 Nature: 
"We obtain daily average temperatures and monthly average precipitation"

Changes from original:
1. Load MONTHLY precipitation files (pr_Amon_*, not pr_day_*)
2. Take MEAN over 12 months per year (not sum over 365 days)
3. This reduces precip values by ~30x, fixing the massive coefficient issue
...
- Precipitation: uses MONTHLY precip flux (pr_Amon_*), converts
  kg m-2 s-1 -> mm/month, then takes MEAN over 12 months.
...
ISO_COL = "ISO"
TAS_VAR_NAME = "tas"
PR_MON_VAR_NAME = "pr"  # Monthly precipitation variable name
...
def build_product_country_climate(
    ...
    # Load precipitation (MONTHLY) - CRITICAL FIX
    # Following Carleton et al. 2022: "monthly average precipitation"
    if precip_path is not None:
        ...
        # Monthly data: select by year-month, not full dates
        ds_p = ds_p.sel(time=slice(f"{YEAR_MIN}-01", f"{YEAR_MAX}-12"))
    else:
        ds_p = None
    ...
    for yr in years:
        ...
        # PRECIPITATION FIX: Use MONTHLY data and take MEAN over 12 months
        # NOT sum over 365 days!
        if ds_p is not None and PR_MON_VAR_NAME in ds_p:
            pr_mon = ds_p[PR_MON_VAR_NAME].sel(
                time=ds_p.time.dt.year == yr_int
            )
            
            if pr_mon.sizes.get("time", 0) == 0:
                print(f"[WARN] {product_label} No precip data for year {yr_int}")
            else:
                # Convert flux to mm/month if needed
                units = (pr_mon.attrs.get("units", "") or "").lower()
                if "kg m-2 s-1" in units or "kg m**-2 s**-1" in units:
                    # kg m-2 s-1 * (86400 s/day * 30.4 days/month) ≈ mm/month
                    pr_mm_month = pr_mon * 86400.0 * 30.4
                else:
                    pr_mm_month = pr_mon
                
                # Take MEAN over 12 months (not sum!)
                # PR1 = mean monthly precip
                # PR2 = mean of squared monthly precip
                ds_sum["PR1"] = pr_mm_month.mean("time", skipna=True)
                ds_sum["PR2"] = (pr_mm_month ** 2).mean("time", skipna=True)
        ...
        # Batch population-weight and aggregate
    ...
    # Rename to CIL output convention
    rename_map = {
