...
# ─── Config ────────────────────────────────────────────────────────────────────
PRODUCT = "MERRA2"  # change if needed: "GMFD", "ERA5-025", "MERRA2", ...
...
def build_regridder(src_like: xr.Dataset, var: str, product: str) -> xe.Regridder:
    ...
    return xe.Regridder(src, dst, method=method, periodic=False, reuse_weights=True)
...
def compute_lr_and_join(product: str, stitched: pd.DataFrame) -> pd.DataFrame:
    ...
    # tas regridding
    rgrd_tas = build_regridder(ds_lr[["tas"]], "tas", product)
    ds_lr_r = regrid_xesmf(ds_lr[["tas"]], rgrd_tas)
    tmean_lr = (
        ds_lr_r.tas.groupby("time.year").mean("time")
                   .mean("year")
                   .transpose("lat", "lon")
                   .compute()
    )
