...
def reshape_for_rolling(ds,wwidth,drop_leaps=True):
    ''' Reshape ds to wrapped-around dayofyear x year
...
def from_cache(fn_in,fn_out,fs,save_kwargs = {'zarr_format':2}):
    ...
    def decorator(func):
        def wrapper(*args,**kwargs):
            ...
            # Save modified data
            utility_save(ds,fn_out,save_kwargs = save_kwargs)
...
def preprocess_models(file_rows,
    ...
    Load model data, regrid to `ref_grid`, rechunk so there's 
    only one chunk in time.

    (this should actually be able to do the obs stuff too?)
    '''
    #-------- Load and preprocess --------
    ...
    if not fs.exists(fns_out['reshp']):
        ...
        try:
            ...
            for row in file_rows.iterrows():
                ...
                if ds_tmp.sizes['time'] % 365 == 0:
                    ...
                    ds_tmp['time'] = [cftime.DatetimeProlepticGregorian(int(row[1]['time'][0:4]),1,1) + pd.Timedelta(days = n)
                                         for n in range(ds_tmp.sizes['time'])]
                ds.append(ds_tmp.drop_vars(['lat_bounds','lon_bounds','lat_bnds','lon_bnds','time_bounds','time_bnds'],
                                           errors='ignore'))
        ...
        ds = ds.drop_vars(['time_bounds','height'],errors='ignore')
        
        # Subset to desired space
        ...
        utility_save(ds,fns_out['rchk'],save_kwargs = {'zarr_format':2})
