...
- Same logic as the monolithic script
- Reads panel + shapes
...
def build_regridder(src_like: xr.Dataset | xr.DataArray) -> xe.Regridder:
    """
    Build an xesmf.Regridder that works for both rectilinear and curvilinear grids.
    - Rectilinear: expects 1D lat/lon
    - Curvilinear (e.g. MERRA2): handles 2D lat/lon arrays
    """
    if "lat" not in src_like.coords or "lon" not in src_like.coords:
        raise ValueError("Source dataset must have lat/lon coords")

    if src_like["lat"].ndim == 2 and src_like["lon"].ndim == 2:
        # Curvilinear: supply dicts directly
        src_grid = {"lon": src_like["lon"].values, "lat": src_like["lat"].values}
    else:
        # Regular rectilinear: coerce & clean
        src_grid = _coerce_latlon(src_like)

    dst_grid = make_target_grid()
...
def regrid_xesmf(ds_in: xr.Dataset, rgrd: xe.Regridder) -> xr.Dataset:
    """
    Apply regridding safely for both rectilinear and curvilinear datasets.
    """
    if "lat" in ds_in.coords and ds_in["lat"].ndim == 1:
