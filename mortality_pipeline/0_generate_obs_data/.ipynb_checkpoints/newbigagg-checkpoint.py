...
# ─── User Settings ─────────────────────────────────────────────────────────────
PRODUCT       = "GMFD" #"MERRA2" #"JRA-3Q" #"GMFD" #ERA = 70, 71 is merra
...
# merge + make chunks consistent *early*
...
# IMPORTANT: single spatial chunk, short time chunks
...
for yr in years:
    ...
    # Sample used to build regridder
    _sample = ds_yr.isel(time=0, drop=True)
    rgrd = build_regridder(_sample)

    # Regrid and chunk BEFORE any math to avoid memory blow-up
    # regrid keeps our established chunking; keep time small, space full-plane
    ...
    # Temperature polynomials (tas is already in °C)
    ...
    # Precipitation polynomials (in mm/day × 86400 already)
    if "pr" in dsy:
    ...
    # Merge after everything is chunked the same way
    ...
    # Ensure PRCP columns are present even if missing
    for k in (1, 2, 3, 4):
