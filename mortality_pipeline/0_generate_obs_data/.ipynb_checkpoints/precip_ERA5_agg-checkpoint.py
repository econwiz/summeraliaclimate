...
# 1a) open the two datasets separately
ds_tas = open_climate(row["tas_filepath"])     # returns a Dataset with .tas
ds_pr  = open_climate(row["precip_filepath"])  # returns a Dataset with your precip var

# 1b) detect which var in the precip file is the precip field
precip_var = None
...
if precip_var is None:
    raise KeyError(f"no recognized precip var in {row['precip_filepath']}")

# 1c) merge them into one Dataset, then chunk over time
ds_full = xr.merge([ds_tas, ds_pr[[precip_var]]]) \
            .chunk({"time": 1})
