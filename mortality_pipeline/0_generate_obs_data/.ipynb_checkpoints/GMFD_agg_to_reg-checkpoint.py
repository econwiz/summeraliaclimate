...
for yr in years:
    ...
    # … build T1–T4, tmean, chunk, aggregate exactly as before …
...
pd.concat(rows, ignore_index=True).fillna(0).to_csv(out_csv, index=False)
print("✅ Done!")
...
prod_df = pd.concat(rows_for_prod, ignore_index=True).fillna(0)
