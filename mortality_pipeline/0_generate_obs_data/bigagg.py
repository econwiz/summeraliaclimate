...
# set products, paths, and chunking for regridding 
...
# Collapse all European country ISOs to a single "EU" code to match the
# shapefile's regional aggregation, which treats EU countries as one unit.
...
# Rebuild the regridder for each year using the first timestep as a source grid template.
# This is intentionally done per-year in case source grid metadata varies across files,
# though for most products the grid is stable and this could be moved outside the loop.
