...
# Carleton et al. global mortality panel. For each *_by_region_year.csv found
# in obs_dir, the script:
...
panel <- read_dta(panel_in) %>%
  ...
  mutate(
    ...
    # Drop any pre-existing climate columns from the panel to avoid duplicate or
    # conflicting variable names when merging in fresh obs data below.
...
if (length(only_products) > 0) {
  ...
  keep <- vapply(obs_files, function(p) {
    # # Standardize product tag formatting in all column names:
    # convert any hyphens to underscores and coerce the product tag to canonical
    # uppercase (e.g. "era5-025" or "era5_025" both become "ERA5_025").
    # This ensures downstream Stata code can rely on consistent column naming.
    base <- basename(p)
...
# Infers the canonical product tag from the obs CSV filename by stripping
# the "_by_region_year.csv" suffix and uppercasing, e.g.:
# "era5_025_by_region_year.csv" -> "ERA5_025"
...
for (obs_csv in obs_files) {
  ...
  merged <- panel %>%
    ...
    # Drop any stray ADM2-average columns that may have been carried over from
    # a previous merge; ADM1-level lr_tavg columns are retained.
...
# --- Post-hoc QC check: inspect missingness in merged MERRA2 panel ---
# Reloads the output file and summarizes which ISOs and ADM1 regions are
# missing temperature polynomial values, to help identify spatial gaps
# in the obs data coverage before running regressions.
