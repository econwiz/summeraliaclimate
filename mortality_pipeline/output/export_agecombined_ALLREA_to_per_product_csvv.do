...
*
* For each product P:
*   beta_P = beta_base + delta_P   (delta_base = 0 for ERA5_025)
...
local STERDIR "$REPO/code/mortality_pipeline/output/age_combined_ALLREA"
local CSVVDIR "$REPO/code/mortality_pipeline/output/age_combined_csvv"
...
forvalues SPEC = 1/5 {
    ...
    forvalues pi = 1/4 {
        ...
        * ------------------------------------------------------------------
        * Reconstruct 4 coefficients (one per poly)
        * ------------------------------------------------------------------
        local ncoef 4
        ...
        forvalues k = 1/4 {
            ...
            * Delta term (zero for base product)
            if `pi' == 1 {
                local has_delta 0
                local id 0
        ...
        * ------------------------------------------------------------------
        ...
        file write csvv "---" _n
        file write csvv "oneline: Age-combined ALLREA (Spec `SPEC') response – `prod_name'" _n
        file write csvv "version: MORTALITY-AGE-COMBINED-ALLREA-SPEC`SPEC'-`prod_name'" _n
        file write csvv "dependencies: `STERFILE'" _n
        file write csvv "description: 4th-order poly OLS pooled ALLREA age-combined; per-product coefficients reconstructed from base+delta" _n
...
di ""
di as res "All age-combined per-product CSVVs written to `CSVVDIR'"
